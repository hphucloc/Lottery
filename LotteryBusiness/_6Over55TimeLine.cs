    ...
    public class _6Over55TimeLine
    {
        ...
        public static Dictionary<DateTime, List<string>> GetColorNextAppear2()
        {
            ...
            */
            ...
            List<DateTime> lstNextDateAppear = new List<DateTime>();
            var data = Common.GetNumbersNextAppear(7, 3, 1);
            var latestNumber = GetSecondLatest6Over55Number().OrderBy(x => Convert.ToInt32(x.LotNumber)).ToList();
        ...
        public static Dictionary<FullLotteryStatistic, Dictionary<DateTime, List<string>>> GetColorNextAppear3(DateTime from, DateTime to)
        {
            ...
            var data = Common.ConvertLotNumberToListFullLotteryStatistic(Get6Over55Number(from, to));
            var data2 = Common.GetNumbersNextAppear(7, 1, 1);
            foreach (var i in data)
            {
                ...
                for (var j = 0; j < i.Numbers.Count(); j++)
                {
                    switch (j)
                    {
                        case 0:
                            ...
                            if (Convert.ToInt32(i.Numbers[j]) > 47 && Convert.ToInt32(i.Numbers[j]) <= 55)
                            {
                                s1 = "47-55";
                            }
                            break;
                ...
                for (var j = 0; j < data2.Count(); j += 7)
                {
                    ...
                    if (Convert.ToInt32(data2[j].LotNumber) > 47 && Convert.ToInt32(data2[j].LotNumber) <= 55)
                        {
                            a1 = "47-55";
                        }
